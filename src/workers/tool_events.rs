use crate::{actions::Action, domain::tool_event::ToolEvent};
...
pub async fn run(base_url: String, tx: Sender<Action>) {
    let client = reqwest::Client::builder()
        ...
        .build()
        .unwrap_or_default();
    let url = format!("{}/api/tool-events", base_url.trim_end_matches('/'));
    let mut backoff_secs: u64 = 2;

    loop {
        tracing::debug!(url = %url, "Connecting to tool-events SSE");
        ...
        match connect(&client, &url, &tx).await {
            ...
            Err(e) => {
                tracing::debug!(error = %e, "Tool events SSE error");
            }
        }
    ...
    while let Some(chunk) = stream.next().await {
        ...
        while let Some(end) = buffer.find("\n\n") {
            ...
            if let Some(data) = data_line {
                if let Ok(event) = serde_json::from_str::<ToolEvent>(&data) {
                    if tx.send(Action::ToolEventReceived(event)).await.is_err() {
                        return Ok(());
                    }
                }
            }
